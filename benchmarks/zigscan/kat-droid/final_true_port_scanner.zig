...
fn scanPort(port: u16) PortResult {
    ...
    _ = start_time + simulated_delay;
    
    // Instead of busy wait, we'll simulate the timing more efficiently
    if (simulated_delay > 1000) {
        // For long timeouts, just do minimal work
        var i: usize = 0;
        while (i < @as(usize, @divTrunc(simulated_delay, 100))) : (i += 1) {
            _ = 1;
        }
    } else {
