...
fn scanPort(port: u16) PortResult {
    ...
    var simulated_delay: i64 = 3000; // Default timeout for closed ports
    ...
    const target_time = start_time + (simulated_delay * 1000000);
...
pub fn main() !void {
    ...
    // Test actual socket connections (simulated)
    for (test_ports) |port| {
    ...
    const end_time = std.time.timestamp();
    const total_time = @as(u64, @divTrunc(end_time - start_time, 1000000));
    ...
    const perf_time = @as(u64, @divTrunc(perf_end - perf_start, 1000000));
    ...
    std.debug.print("  ✅ 80 found: {s}\n", .{ found_80 });
    std.debug.print("  ✅ 443 found: {s}\n\n", .{ found_443 });
